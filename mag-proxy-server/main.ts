...
async function fetchFantoriaArticles(): Promise<Article[]> {
  logger.info("Fangoria 기사 가져오는 중...");

  try {
    const articles = await scrapeFangoriaArticles();
    logger.info(`${articles.length}개의 기사 파싱 완료`);
    return articles;
  } catch (error) {
    logger.error('Fangoria 기사 가져오기 오류:', error);
    throw error;
  }
}
...
async function handleGetArticles(ctx: Context): Promise<void> {
  ...
  try {
    ...
    if (!forceRefresh) {
      ...
      if (cachedArticles) {
        logger.info("Serving cached articles");
    ...
    logger.error('Error in handleGetArticles:', error);
  }
}
